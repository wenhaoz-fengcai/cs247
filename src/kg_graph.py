...
class KG(Graph):
    ...
    def __clean_nodes(self, nodes):
        ...
        Mask off {"N": [(word, label)], "C": [(word, label)]}
        Args:
            nodes: dictionary of nodes. 

        Return:
            Returns a dictionary of nodes with {"N": [(word, label)], "C": [(word, label)]} excluded.
        """
        ret = dict()
        ret["K"] = list(nodes["K"])
        ret["N"] = list()
    ...
    def __clean_edges(self, edges):
        """Private class method
        Remove all edges whose both ends are not in self.nodes["K"]
        Args:
            edges: set of tuples

        Return:
            Returns a set of edges whose both ends are in self.nodes["K"]
        """
        ret = set()
        for item in edges:
            if (self.__in_nodes(self.nodes["K"], item[0]) 
                and self.__in_nodes(self.nodes["K"], item[1]) ):
                ret.add(item)
        return ret
