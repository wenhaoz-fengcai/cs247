...
class Embedding(object):
    ...
    def __init__(self, d=10):
        ...
        self.read = Reader()
        # self.news_list = ["Trump is dumb", "Wenhao is at risksciences", "Trump at Whitehouse",
        #                   "Kate Middleton and Prince William", "Noor Nakhaei studies at UCLA", "The world sucks",
        #                   "and I'm pretty sure that's it!", "UCLA is a location", "hey is an organization",
        #                   "Trump is UCLA", "The UCLA is UNICEF", "UNICEF and hi", "UCLA is a location",
        #                   "football is an organization"]
        ...
        self.S = pd.DataFrame(randint(1, 10), index=self.entities, columns=range(0, d))
        self.T = pd.DataFrame(randint(1, 10), index=self.words, columns=range(0, d))
    ...
    def embedding_update(self, s, t, g, k=3):
        ...
        if s == 1 and t == 1:  # S, T, G_ec
            ...
            for i in range(k + 1):
                ...
                sum = sum + b
            c = np.log(sum)
            #               d = inv(self.S.loc[sampled_node_a])
